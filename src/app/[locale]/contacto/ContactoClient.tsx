...
import type { SalesContact } from "@/lib/site-settings-api"
...
interface ContactoClientProps {
    schedule: string;
    location: string;
    phoneWhatsappDigits: string;
    whatsappOnlyDigits: string;
    email: string;
    salesContacts: (SalesContact & { displayTitle: string; digits: string })[];
}
                                ...
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <p className="text-gray-600 text-sm whitespace-pre-line">{schedule}</p>
                                        </div>
                                        <div className="pt-4 border-t border-gray-50">
                                            <p className="font-bold text-gray-900">{t('info.location_title')}</p>
                                            <div className="flex items-start gap-2 mt-1">
                                                <MapPin className="w-4 h-4 text-gray-400 mt-1 shrink-0" />
                                                <p className="text-gray-600 text-sm whitespace-pre-line leading-relaxed">{location}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        ...
                                        <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                                            ...
                                            <div>
                                                ...
                                                <a href={`https://wa.me/${phoneWhatsappDigits}`} className="block text-brand-dark font-bold hover:text-brand-gold transition-colors">
                                                    +{phoneWhatsappDigits}
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                                            ...
                                            <div>
                                                ...
                                                <a href={`https://wa.me/${whatsappOnlyDigits}`} className="block text-brand-dark font-bold hover:text-brand-gold transition-colors">
                                                    +{whatsappOnlyDigits}
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                                            ...
                                            <div>
                                                ...
                                                <a href={`mailto:${email}`} className="text-brand-dark font-bold hover:text-brand-gold transition-colors">
                                                    {email}
                                                </a>
                                            </div>
                                        </div>
                                ...
                                <div className="bg-brand-dark text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
                                    ...
                                    <div className="space-y-6 relative z-10">
                                        {salesContacts.map((contact, index) => (
                                            <div key={`${contact.email ?? contact.phone ?? contact.displayTitle}-${index}`}>
                                                <p className="text-xs text-gray-400 mb-1">{contact.displayTitle}</p>
                                                {contact.digits && (
                                                    <p className="text-lg font-bold">+{contact.digits}</p>
                                                )}
                                                {contact.email && (
                                                    <a href={`mailto:${contact.email}`} className="text-brand-gold text-sm hover:underline">{contact.email}</a>
                                                )}
                                            </div>
                                        ))}
                                    </div>
